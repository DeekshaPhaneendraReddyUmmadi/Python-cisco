'''
    salary manager using list with crud operations
'''

salaries = []

def createSalary(salary):
    global salaries
...
def readBySalary(salary):
    for i in range(len(salaries)):
        if salary == salaries[i]:
            return i
    return -1

def updateBySalary(oldSalary, newSalary):
    global salaries
    index = readBySalary(oldSalary)
    salaries[index] = newSalary

def deleteBySalary(salary):
    global salaries
